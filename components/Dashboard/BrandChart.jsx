...
const BrandChart = () => {
  ...
  const labels = sales.map((brand) => brand.Brand.slice(0, 6));
  const imageUrls = sales.map((brand) => brand.imageUrl);
  const sale = sales.map((brand) => brand.sales);

  useEffect(() => {
    const calculateSalesByCategory = () => {
      const updatedBrandData = brandData.map((brand) => {
        const salesByBrand = ordersData.reduce((count, order) => {
          const matchingItems = order.items.filter(
            (product) => product && product.Brand === brand.Brand
          );
          return count + matchingItems.length;
        }, 0);
        return { ...brand, sales: salesByBrand };
      });
      setSales(updatedBrandData);
    };

    calculateSalesByCategory();
  }, [ordersData, brandData]);
      ...
      <div className="flex p-2 items-start justify-between gap-3 text-white">
        <div className="flex w-full flex-wrap gap-3 sm:gap-5">
          ...
          <div className="flex min-w-47.5">
            ...
            <div className="w-full">
              <p className="font-bold text-gray-600">Number of Brands</p>
              <p className="text-sm font-medium">{brandData.length}</p>
            </div>
          </div>
        </div>
      </div>
