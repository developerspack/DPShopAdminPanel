...
const AddEditCategory = ({ id }) => {
  const { document } = FetchDocument("category", id);
  const [form, setForm] = useState(initialState);
  const [fileType, setFileType] = useState(true);
  const [isLoading, setIsLoading] = useState(false);
  const [selectedFile, setSelectedFile] = useState(null);
  const filePickerRef = useRef(null);
  const { Category, imageUrl } = form;
    ...
    if (Category && selectedFile) {
      if (!id) {
        if (fileType === false) {
          ...
          try {
            uploadDocument("category", form, selectedFile);
        ...
        if (fileType === false) {
          ...
          try {
            UpdateDcoument("category", form.id, selectedFile, imageUrl, form);
            router.push("/CustomizeCategory");
            setIsLoading(false);
          } catch (error) {
            toast.error(error.message);
            setIsLoading(false);
